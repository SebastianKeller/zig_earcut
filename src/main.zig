...
pub const Earcut = struct {
    arena: std.heap.ArenaAllocator,

    pub fn init(allocator: *std.mem.Allocator) Earcut {
        return Earcut{
            .arena = std.heap.ArenaAllocator.init(allocator),
        };
    }

    pub fn deinit(self: Earcut) void {
        self.arena.deinit();
    }

    pub fn earcut(self: *Earcut, data: []f64, hole_indicies: ?[]usize, dim: usize) ![]usize {
    ...
    pub fn flatten(comptime dim: usize, data: [][][dim]f64, allocator: *std.mem.Allocator) !Flattened {
        ...
        var vertices = try allocator.alloc(f64, vertices_count);
    ...
    fn earcutLinked(
        ...
        if (pass == -1 and size != std.math.f64_min)
        ...
        while (e.prev != e.next) {
            ...
            if (if (size != std.math.f64_min) isEarHashed(e, minX, minY, size) else isEar(e)) {
    ...
    fn cureLocalIntersections(self: *Earcut, start: *Node, triangles: *std.ArrayList(usize), dim: usize) !*Node {
        var s = start;
        var p = s;
        while (true) {
            var a = p.prev;
    ...
    fn isEarHashed(ear: *const Node, minX: f64, minY: f64, size: f64) bool {
        var a = ear.prev;
        var b = ear;
        var c = ear.next;
    ...
    fn zOrder(x: f64, y: f64, minX: f64, minY: f64, size: f64) f64 {
        ...
        return @intToFloat(f64, lx | (ly << 1));
    }

    fn indexCurve(start: *Node, minX: f64, minY: f64, size: f64) void {
        var p = start;
        while (true) {
            if (p.z == std.math.f64_min)
    ...
    fn eliminateHoles(self: *Earcut, data: []f64, holeIndices: []usize, outerNode: *Node, dim: usize) !*Node {
    ...
    fn eliminateHole(self: *Earcut, hole: *Node, outerNode: *Node) !void {
    ...
    fn splitPolygon(self: *Earcut, a: *Node, b: *Node) !*Node {
    ...
    fn findHoleBridge(hole: *Node, outerNode: *Node) ?*Node {
        ...
        var mx = m.?.x;
        var my = m.?.y;
        var tanMin: f64 = std.math.f64_max;
    ...
    fn linkedList(self: *Earcut, data: []f64, start: usize, end: usize, dim: usize, clockwise: bool) !?*Node {
        var last: ?*Node = null;
    ...
    fn insertNode(self: *Earcut, i: usize, x: f64, y: f64, last: ?*Node) !*Node {
    ...
    fn signedArea(data: []f64, start: usize, end: usize, dim: usize) f64 {
        var sum: f64 = 0.0;
        var j = end - dim;
        var i = start;
        while (i < end) : (i += dim) {
    ...
    fn area(p: *const Node, q: *const Node, r: *const Node) f64 {
